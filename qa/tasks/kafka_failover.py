    ...
    for (client, _) in config.items():
        ...
        ctx.cluster.only(client).run(
            args=['sed', '-i\""', '"s/broker.id=0/broker.id=1/g"', '{tdir}/config/{second_broker_config_name}'.format(tdir=kafka_dir, second_broker_config_name=second_broker_config_name), run.Raw('&&'),
                  'sed', '-i\""', '"s/listeners=PLAINTEXT:\/\/:19092/listeners=PLAINTEXT:\/\/localhost:19092 /g"', '{tdir}/config/{second_broker_config_name}'.format(tdir=kafka_dir, second_broker_config_name=second_broker_config_name), run.Raw('&&'),
                  'sed', '-i\""', '"s/#advertised.listeners=PLAINTEXT:\/\/:9092/advertised.listeners=PLAINTEXT://localhost:19092/g"', '{tdir}/config/{second_broker_config_name}'.format(tdir=kafka_dir, second_broker_config_name=second_broker_config_name), run.Raw('&&'),
                  'sed', '-i\""', '"s/log.dirs=\/tmp\/kafka-logs/log.dirs={}"'.format(second_broker_data_logs_escaped), '{tdir}/config/{second_broker_config_name}'.format(tdir=kafka_dir, second_broker_config_name=second_broker_config_name),
            ]
        )
    ...
    for (client,_) in config.items():
        ...
        ctx.cluster.only(client).run(
            args=['cd', '{tdir}/bin'.format(tdir=kafka_dir), run.Raw('&&'),
             ...
             '{tir}/config/server.properties'.format(tdir=kafka_dir), run.Raw('&&'),
             'LOG_DIR={second_broker_java_log_dir} ./kafka-server-start.sh'.format(second_broker_java_log_dir=second_broker_java_log_dir),
             '{tir}/config/server2.properties'.format(tir=kafka_dir),
             run.Raw('&'), 'exit'
            ],
        )
